	...
	switch(dialogid)
	{
		...
		case DIALOG_ADD_GLOC:
		{
			if(response)
			{
				...
				mysql_format(GPSDB, Query, sizeof(Query), "SELECT * FROM `gpsdb` WHERE `LocationName` = '%e'", inputtext);
				...
				mysql_format(GPSDB, Query2, sizeof(Query2), "INSERT INTO `gpsdb` (`LocationName`, `PositionX` , `PositionY` , `PositionZ`, `InteriorID`) VALUES ('%s', '%f', '%f', '%f', '%d')", inputtext, gInfo[playerid][Pos][0], gInfo[playerid][Pos][1], gInfo[playerid][Pos][2], gInfo[playerid][Interior]);
		...
		case DIALOG_DEL_GLOC:
		{
			if(response)
			{
				mysql_format(GPSDB, Query, sizeof(Query),"DELETE FROM `gpsdb` WHERE `LocationName` = '%e'", inputtext);
		...
		case DIALOG_GPS:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						new Cache:result = mysql_query(GPSDB,"SELECT LocationName, PositionX, PositionY, PositionZ FROM `gpsdb` WHERE `S.No` > -1");
						if(!cache_num_rows())
						{
							cache_delete(result);
